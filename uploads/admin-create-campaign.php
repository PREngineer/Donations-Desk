...
$required_fields = array('username', 'info', 'goal', 'end', 'category', 'paypal');
...
foreach($_POST as $key=>$value)
{
  ...
  {
    // Redirect with error
    header('Location: ../en/admin-create-campaign.php?e4=1&goal=' . $_POST['goal'] . '&info=' . $_POST['info'] . '&end=' . 
            $_POST['end'] . '&category=' . $_POST['category'] . '&paypal=' . $_POST['paypal'] . '&username=' . $_POST['username']);
...
if(checkdate($m, $d, $y) == false)
{
    ...
    // Redirect with error
    header('Location: ../en/admin-create-campaign.php?e5=1&goal=' . $_POST['goal'] . '&info=' . $_POST['info'] . '&end=' . 
            $_POST['end'] . '&category=' . $_POST['category'] . '&paypal=' . $_POST['paypal'] . '&username=' . $_POST['username']);
...
{
    // Redirect with error
    header('Location: ../en/admin-create-campaign.php?e6=1&goal=' . $_POST['goal'] . '&info=' . $_POST['info'] . '&end=' . 
            $_POST['end'] . '&category=' . $_POST['category'] . '&paypal=' . $_POST['paypal'] . '&username=' . $_POST['username']);
...
if($_FILES['file']['error'] == 4)
{
    ...
    echo 'Path is: ' . $path . '<br>User is: ' . $username . '<br>Info is: ' . $info . '<br>End Date is: ' . $end . '<br>Goal is: ' . 
    $goal . '<br>Category is: ' . $category . '<br>Paypal is: ' .$paypal;

    // Actual MySQL Query that Updates the information
    mysql_query("INSERT into `Campaigns` (`username`, `info`, `end`, `goal`, `category`, `paypal`) 
        VALUES ('$username', '$info', '$end', '$goal', '$category', '$paypal')");

    // Redirect to Success Message
    $address = '../en/admin-create-campaign.php?success=true';
...
else 
{
    ...
    {
    	// Redirect with error
        header('Location: ../en/admin-create-campaign.php?e1=1&goal=' . $_POST['goal'] . '&info=' . $_POST['info'] . '&end=' . 
            $_POST['end'] . '&category=' . $_POST['category'] . '&paypal=' . $_POST['paypal'] . '&username=' . $_POST['username']);
	...
	{
    	// Redirect with error
        header('Location: ../en/admin-create-campaign.php?e2=1&goal=' . $_POST['goal'] . '&info=' . $_POST['info'] . '&end=' . 
            $_POST['end'] . '&category=' . $_POST['category'] . '&paypal=' . $_POST['paypal'] . '&username=' . $_POST['username']);
    ...
    { 
        // Redirect with error
        header('Location: ../en/admin-create-campaign.php?e3=1&goal=' . $_POST['goal'] . '&info=' . $_POST['info'] . '&end=' . 
            $_POST['end'] . '&category=' . $_POST['category'] . '&paypal=' . $_POST['paypal'] . '&username=' . $_POST['username']);
    ...
    else
    {
        ...
        // Actual MySQL Query that Updates the information
        mysql_query("INSERT into `Campaigns` (`username`, `info`, `end`, `goal`, `category`, `campaign-logo`, `paypal`) 
            VALUES ('$username', '$info', '$end', '$goal', '$category', '$path', '$paypal')");
