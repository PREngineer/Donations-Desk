...
if (!empty($_FILES['file'])) 
{
    ...
    {
        // Redirect with error
        header('Location: ../en/admin-manage-nfpo.php?e1=1&edit=6&id=' . $_POST['id'] . '&doc=' . $_POST['doc']);
    ...
    // Check file size
    // Max Allowed for picture 2 MB (size is also in bytes)
    if($myFile['size'] > 2097152) 
    {
        // Redirect with error
        header('Location: ../en/admin-manage-nfpo.php?e5=1&edit=6&id=' . $_POST['id'] . '&doc=' . $_POST['doc']);
    ...
    {
        // Redirect with error
        header('Location: ../en/admin-manage-nfpo.php?e3=1&edit=6&id=' . $_POST['id'] . '&doc=' . $_POST['doc']);
    ...
    if ($success == false) 
    { 
        $address = '../en/admin-manage-nfpo.php?e4=1&edit=6&id=' . $_POST['id'] . '&doc=' . $_POST['doc'];
        // Redirect with error
    ...
    else
    {
        ...
        $address = '../en/admin-manage-nfpo.php?success=true&edit=6&id=' . $_POST['id'] . '&doc=' . $_POST['doc'];
        header('Location: ' . $address);
        exit();
    }
}
