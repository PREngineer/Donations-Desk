...
foreach($_POST as $key=>$value)
{
  ...
  {
    // Redirect with error
    header('Location: ../en/user-edit-campaign.php?e4=1&id=' . $_POST['id']);
...
if(checkdate($m, $d, $y) == false)
{
    ...
    // Redirect with error
    header('Location: ../en/user-edit-campaign.php?e5=1');
...
{
    // Redirect with error
    header('Location: ../en/user-edit-campaign.php?e6=1');
...
if($_FILES['file']['error'] == 4)
{
    // Save Path to DB
        ...
        mysql_query("UPDATE `Campaigns` SET `info`='$info', `end`='$end', `goal`='$goal', `category`='$category', `paypal`='$paypal' WHERE `id` = $id");

        // Redirect to Success Message
        $address = '../en/user-edit-campaign.php?success=true&id=' . $id;
...
else
{
    ...
    {
    	// Redirect with error
        header('Location: ../en/user-edit-campaign.php?e1=1&goal=' . $_POST['goal'] . '&info=' . $_POST['info'] . '&end=' . $_POST['end'] . '&category=' . $_POST['category'] . '&paypal=' . $_POST['paypal']);
	...
	{
    	// Redirect with error
        header('Location: ../en/user-edit-campaign.php?e2=1&goal=' . $_POST['goal'] . '&info=' . $_POST['info'] . '&end=' . $_POST['end'] . '&category=' . $_POST['category'] . '&paypal=' . $_POST['paypal']);
    ...
    { 
        // Redirect with error
        header('Location: ../en/user-edit-campaign.php?e3=1&goal=' . $_POST['goal'] . '&info=' . $_POST['info'] . '&end=' . $_POST['end'] . '&category=' . $_POST['category'] . '&paypal=' . $_POST['paypal']);
    ...
    else
    {
        ...
        echo 'Path is: ' . $path . '<br>User is: ' . $username . '<br>Info is: ' . $info . '<br>End Date is: ' . $end . '<br>Goal is: ' . $goal . '<br>Category is: ' . $category . '<br>Paypal is: ' .$paypal;

        // Actual MySQL Query that Updates the information
        mysql_query("UPDATE `Campaigns` SET `info`='$info', `end`='$end', `goal`='$goal', `category`='$category', `campaign-logo`='$path', `paypal`='$paypal' WHERE `id` = '$id'");
