<?php

// Get the organization names
$organizations = mysql_query("SELECT DISTINCT `org-id` 
								FROM `osfl-donations` 
								WHERE `date` LIKE '".$_POST['year']."-".$_POST['month']."%'");

$output .= '
<b><u>Totales de OSFL</u></b><br>
<b>Fecha Cubierta: </b>' . $_POST['month'] . '/' . $_POST['year'] . '<br>
<b>Reporte Generado: </b>' . date("D, F d Y") . '<br><br>';

$i = 0;

// Table Displaying all NFPOs (for Export to excel)
$output .= '
<table align="left" border="1">
  <tbody>
	<center>

	  	<tr bgcolor="#000000">
			<td width="100">
			<font color="#FFFFFF"><b>#</b></font>
			</td>
			<td width="300">
			<font color="#FFFFFF"><b>Nombre</b></font>
			</td>
			<td width="300">
			<font color="#FFFFFF"><b>Donaciones</b></font>
			</td>
			<td width="300">
			<font color="#FFFFFF"><b>Total Donado</b></font>
			</td>
		</tr>';

// Go over every Organization
while ( $i < mysql_num_rows($organizations) ) 
{
	$id = mysql_result($organizations, $i, "org-id");

	// Get the Organization Name
	$name = mysql_result( mysql_query("SELECT `organization-name` FROM `OSFL` WHERE `id` = ".$id." ORDER BY `organization-name`"), 0 );

	// Get the Totals
	$totals = mysql_fetch_assoc( mysql_query("SELECT COUNT(amount) as total, 
												SUM(amount) as donated 
												FROM `osfl-donations` 
												WHERE `org-id` = ".$id." 
												AND `date` LIKE '".$_POST['year']."-".$_POST['month']."%'") );

	$output .= '
		<tr class="block"';
		// Uneven rows are gray
		if($i % 2 != 0)
		{
			$output .= ' bgcolor="#9F9B9B"';
		}
		else
		{
			$output .= ' bgcolor="#FEDD8C"';
		}
	$output .=	'>
			<td>'.
			($i+1)
			.'</td>
			<td width="300">'.
			$name
			.'</td>
			<td width="300">'.
			$totals['total']
			.'</td>
			<td width="300">$'.
			$totals['donated']
			.'</td>
		</tr>
	';

	$i++;
}

$output .= '
	</center>
  </tbody>
</table>';

// Show the actual content
echo $output;

?>

<br><br>

<table>
	<tr>
		<td align='center'>

			<form action = "/donationsdesk/excel/export.php" method = "post">
			    <input type = "hidden" name = "body" value = '<?php echo $output; ?>'>
			    <input type = "submit" name = "Excel" Value = "Exportar a Excel">
			</form>

		</td>
	</tr>
</table>