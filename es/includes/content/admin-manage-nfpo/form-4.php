<?php
  ...
  */
  $fetched_data = admin_nfpo_data($_GET['id'], 'username', 'vision', 'mission', 'target', 'impact', 'projections', 'services');
  ...
  else if(empty($errors) === false)
  {
    ...
    if($success === true)
    {
      echo '<center><font color="green"><p>¡Cambios guardados!</p></font></center>';
    }
?>
...
<table border="1">
  <tbody>
    ...
    <tr>
      ...
      <td width="150">
        <a href="admin-manage-nfpo.php?edit=3&id=<?php echo $_GET['id']?>">Donaciones</a>
      </td>
      <td width="150">
    ...
    <form class="login-form" enctype="multipart/form-data" action="" method="POST">
      ...
      <tbody>
        ...
        <tr>
          <th>Visión [*]</th>
          <td>
            <textarea class="w-input" name="vision" rows="2" cols="40" placeholder="Entre su Visión"><?php if( empty($_POST) === true ) print_r($fetched_data['vision']); else print_r($_POST['vision']); ?></textarea>
          </td>
        </tr>
        ...
        <tr>
          <th>Misión [*]</th>
          <td>
            <textarea class="w-input" name="mission" rows="2" cols="40" placeholder="Entre su Misión"><?php if( empty($_POST) === true ) print_r($fetched_data['mission']); else print_r($_POST['mission']); ?></textarea>
          </td>
        </tr>
        
        <tr>
          <th>Servicios Provistos [*]</th>
          <td>
            <textarea class="w-input" name="services" rows="2" cols="40" placeholder="Liste los Servicios que provee"><?php if( empty($_POST) === true ) print_r($fetched_data['services']); else print_r($_POST['services']); ?></textarea>
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            <textarea class="w-input" name="projections" rows="2" cols="40" placeholder="Entre Proyecciones"><?php if( empty($_POST) === true ) print_r($fetched_data['projections']); else print_r($_POST['projections']); ?></textarea>
          </td>
        </tr>
        ...
        <tr>
          <th>Impacto Poblacional [*]</th>
          <td>
            <select size="1" name="impact">
            <option value="" <?php if( empty($_POST) === true ) { if ($fetched_data['impact'] == '')    echo ' selected = "selected"'; } else { if($_POST['impact'] == '') echo ' selected = "selected"';} ?> >Seleccione</option>
